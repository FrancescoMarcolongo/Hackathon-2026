"""Run storage-Kerr drift tracking with an explicit fifth compensation knob."""
...
from storage_kerr_compensation import (
    DEFAULT_STORAGE_KERR_BOUNDS,
    StorageKerrCompensationConfig,
    apply_storage_kerr_compensation,
...
BOUNDS_5D = DEFAULT_STORAGE_KERR_BOUNDS
CONTROL_SUFFIXES = ("g2_real", "g2_imag", "eps_d_real", "eps_d_imag", "storage_kerr")
WARMUP_EPOCH = 15
CAT_REFERENCE_SCALES = np.array([0.35, 0.25, 0.60, 0.50], dtype=float)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sigma0", type=float, default=0.36)
    ...
    parser.add_argument(
        "--cat-reference-weight",
        type=float,
        default=2.0,
        help="Mild prior that keeps the four original cat controls near x_ref in the 5D comparison.",
    )
    parser.add_argument(
        "--x-reference",
        type=float,
...
def build_storage_config(args: argparse.Namespace) -> StorageKerrCompensationConfig:
    ...
    return StorageKerrCompensationConfig(**kwargs)
...
def cat_reference_penalty(
    ...
    *,
    ...
    if weight <= 0:
        return 0.0
    x = np.asarray(x_cmd, dtype=float)[:4]
    x_ref = np.asarray(storage_cfg.x_reference, dtype=float)
    scaled = (x - x_ref) / CAT_REFERENCE_SCALES
    return float(weight * np.dot(scaled, scaled))


def evaluate_x_kerr5(
    x_cmd: np.ndarray,
    *,
    epoch: int,
    ...
    state = apply_storage_kerr_compensation(x_cmd, epoch, storage_cfg, BOUNDS_5D)
    ...
    cat_penalty = cat_reference_penalty(
        state["x_cmd_clipped"],
        storage_cfg,
        weight=float(cat_reference_weight),
    )
    reward["reward"] = float(reward["reward"]) - signature_penalty - cat_penalty
    ...
    row = {
        ...
        "storage_kerr_drift": float(state["kerr_drift"]),
        "residual_kerr": float(state["residual_kerr"]),
        ...
        "cat_reference_penalty": float(cat_penalty),
    }
    for key in (
        "T_X",
        "T_Z",
...
def run_storage_kerr5_tracking(
    *,
    sim_cfg: SimulationConfig,
    reward_cfg: RewardConfig,
    storage_cfg: StorageKerrCompensationConfig,
    ...
    clear_measure_cache()
    mean0 = np.array([*OPTIMIZATION_START_X.copy(), kerr_compensation_drift(0, storage_cfg)], dtype=float)
    ...
    for epoch in range(1, int(epochs) + 1):
        ...
        if epoch == 1 or epoch % 5 == 0 or epoch == epochs:
            row = history[-1]
            print(
                f"storage-kerr5 epoch={epoch:03d}/{epochs} "
                f"bias={row['bias']:.4g} reward={row['reward']:.3f} "
                f"track_l2={row['tracking_error_l2']:.4g} "
                f"K_resid={row['effective_storage_kerr']:.4g} "
...
def compute_summary_metrics(
    ...
    *,
    ...
    residual = np.array([float(row["storage_kerr_residual_abs"]) for row in post], dtype=float)
    ...
    return {
        ...
        "median_storage_kerr_residual_abs_after_warmup": _finite_median(residual),
        "final_storage_kerr_residual_abs": float(final["storage_kerr_residual_abs"]),
        ...
        "tracking_good": bool(
            ...
            and _finite_median(tracking) < 0.15
            and np.isfinite(_finite_median(residual))
            and _finite_median(residual) < 0.10
...
def write_decay_snapshots(
    ...
    *,
    ...
    path = FIGURES / "storage_kerr5_optimized_decay_fits_final.png"
    plot_decay_fit(final_result, path, "Storage-Kerr 5-knob tracked final decay fits")
    figures["storage_kerr5_optimized_decay_fits_final"] = str(path)
    if skip_optional:
        return figures
    selected = [
        ("epoch_000", history[0]),
    ...
    for label, row in selected:
        ...
        snap_path = FIGURES / f"storage_kerr5_optimized_decay_fits_{label}.png"
        plot_decay_fit(result, snap_path, f"Storage-Kerr 5-knob decay fits {label}")
        figures[f"storage_kerr5_optimized_decay_fits_{label}"] = str(snap_path)
    return figures
...
def write_report(
    ...
    *,
    ...
    good_text = (
        "Yes. The explicit Kerr compensation command tracks the oscillating target, the "
        "residual physical Kerr remains small after warm-up, and eta stays in the target band."
        if summary["tracking_good"]
        else "Not fully. The five-knob run completed, but the post-warm-up residual Kerr, "
        "tracking error, or success-band fraction is below the requested threshold."
    )
    lines = [
    ...
    lines += [
        ...
        f"- Kerr reference: `{storage_cfg.kerr_reference:g}`",
        "",
        ...
        "```",
        "",
        "The optimizer is not given `K_drift(e)` or `x_opt_true(e)` when choosing candidates. "
        "It only sees measured rewards/lifetimes/bias under the current residual Kerr. For the "
        "five-knob comparison I also apply a mild stationary-calibration prior on the four "
        "original cat controls, with no epoch-dependent drift information, so the physically "
        "informative Kerr knob is preferred over equivalent four-control retunings.",
        "",
        f"- cat-control stationary prior weight: `{cat_reference_weight:g}`",
        "",
        "Bounds check:",
        "",
        "```json",
    ...
    lines += [
        "",
        "The parameter plot keeps the original four cat controls in the top panel and puts "
        "the physically informative Kerr compensation knob in the bottom panel. The bottom "
        "panel is the clean oscillatory target that the four-knob manifold lacks.",
        "",
        "## Is the result good?",
        "",
        good_text,
        ...
        "- median absolute residual storage Kerr after warm-up: "
        f"`{summary['median_storage_kerr_residual_abs_after_warmup']:.6g}`",
        f"- final absolute residual storage Kerr: `{summary['final_storage_kerr_residual_abs']:.6g}`",
        ...
        "",
        "This is still a deterministic, noise-free Kerr benchmark. The natural next step is "
        "a combined detuning-plus-Kerr drift and an uncertainty-aware reward under stochastic "
        "measurement noise.",
        "",
        "## Optimizer details",
        "",
        ...
        _json_dumps(
            {
                ...
                "cat_reference_weight": float(cat_reference_weight),
            }
        ),
        "```",
    ]
...
def main() -> None:
    ...
    epochs = int(args.epochs if args.epochs is not None else (35 if args.quick else 100))
    ...
    print(f"Storage-Kerr 5D simulation config: {asdict(sim_cfg)}")
    print(f"Reward config: {asdict(reward_cfg)}")
    print(f"Storage-Kerr compensation config: {asdict(storage_cfg)}")
    ...
    history, samples, optimizer_metadata = run_storage_kerr5_tracking(
        sim_cfg=sim_cfg,
        reward_cfg=reward_cfg,
        storage_cfg=storage_cfg,
        reference_metrics=reference_metrics,
    ...
    write_csv(RESULTS / "storage_kerr5_optimization_history.csv", history)
    write_csv(RESULTS / "storage_kerr5_epoch_samples.csv", samples)
    write_json(RESULTS / "storage_kerr5_summary_metrics.json", summary)
    write_json(
        RESULTS / "storage_kerr5_final_candidate.json",
        {
    ...
    figures = {
        "storage_kerr5_bias_vs_epoch": str(FIGURES / "storage_kerr5_bias_vs_epoch.png"),
        "storage_kerr5_lifetimes_vs_epoch": str(FIGURES / "storage_kerr5_lifetimes_vs_epoch.png"),
        "storage_kerr5_reward_or_loss_vs_epoch": str(FIGURES / "storage_kerr5_reward_or_loss_vs_epoch.png"),
        "storage_kerr5_parameters_vs_epoch": str(FIGURES / "storage_kerr5_parameters_vs_epoch.png"),
        "storage_kerr5_signal_vs_epoch": str(FIGURES / "storage_kerr5_signal_vs_epoch.png"),
        "storage_kerr5_tracking_error_vs_epoch": str(FIGURES / "storage_kerr5_tracking_error_vs_epoch.png"),
        "storage_kerr5_effective_parameters_vs_epoch": str(FIGURES / "storage_kerr5_effective_parameters_vs_epoch.png"),
    }
    plot_kerr5_bias_vs_epoch(
        history,
        ...
        FIGURES / "storage_kerr5_bias_vs_epoch.png",
    )
    plot_kerr5_lifetimes_vs_epoch(history, FIGURES / "storage_kerr5_lifetimes_vs_epoch.png")
    plot_kerr5_reward_vs_epoch(history, FIGURES / "storage_kerr5_reward_or_loss_vs_epoch.png")
    plot_kerr5_parameters_vs_epoch(history, FIGURES / "storage_kerr5_parameters_vs_epoch.png")
    plot_kerr5_signal_vs_epoch(history, FIGURES / "storage_kerr5_signal_vs_epoch.png")
    plot_kerr5_tracking_error_vs_epoch(history, FIGURES / "storage_kerr5_tracking_error_vs_epoch.png")
    plot_kerr5_effective_parameters_vs_epoch(history, FIGURES / "storage_kerr5_effective_parameters_vs_epoch.png")
    figures.update(
    ...
    command = "python run_storage_kerr_compensation_tracking.py"
    if args.quick:
        command += " --quick"
    ...
    if args.cat_reference_weight != 2.0:
    ...
    write_report(
        ROOT / "storage_kerr_compensation_validation_report.md",
        command=command,
        sim_cfg=sim_cfg,
        reward_cfg=reward_cfg,
        storage_cfg=storage_cfg,
    ...
    print("\nStorage-Kerr 5D tracking summary")
    ...
    print(
        "median post-warmup Kerr residual="
        f"{summary['median_storage_kerr_residual_abs_after_warmup']:.6g}"
    )
    print(f"Saved report to {ROOT / 'storage_kerr_compensation_validation_report.md'}")
