"""Plots for storage-detuning drift compensation experiments."""
...
def plot_detuning_bias_vs_epoch(
    rows: list[dict],
    target_bias: float,
    bias_tol_rel: float,
    warmup_epoch: int,
...
def plot_detuning_lifetimes_vs_epoch(rows: list[dict], path: Path) -> None:
    set_style()
    epochs = _epochs(rows)
    fig, ax = plt.subplots(1, 1, figsize=(7, 4), dpi=200)
...
def plot_detuning_reward_vs_epoch(rows: list[dict], path: Path) -> None:
    set_style()
    epochs = _epochs(rows)
...
def plot_detuning_parameters_vs_epoch(rows: list[dict], path: Path) -> None:
    set_style()
    epochs = _epochs(rows)
    ...
    det_ax.plot(
        epochs,
        _series(rows, "command_storage_detuning"),
        lw=2.0,
        color="#c04f15",
        label="storage detuning command",
    )
    det_ax.plot(
        epochs,
        _series(rows, "true_opt_storage_detuning"),
        lw=1.7,
        color="#c04f15",
        ls="--",
        label="storage detuning true optimum",
    )
    det_ax.axhline(0.0, color="#1f1f1f", lw=0.8, alpha=0.35)
    det_ax.set_xlabel("Epoch")
    det_ax.set_ylabel("Detuning")
    det_ax.grid(alpha=0.3)
    det_ax.legend(frameon=False)
    _save(fig, path)


def plot_detuning_signal_vs_epoch(rows: list[dict], path: Path) -> None:
    set_style()
    epochs = _epochs(rows)
    fig, ax = plt.subplots(1, 1, figsize=(7, 4), dpi=200)
    ax.plot(
        epochs,
        _series(rows, "drift_storage_detuning"),
        lw=2.0,
        color="#c04f15",
        label="storage detuning drift",
    )
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Storage detuning")
    ax.grid(alpha=0.3)
    ax.legend(frameon=False)
    _save(fig, path)


def plot_detuning_tracking_error_vs_epoch(rows: list[dict], path: Path) -> None:
    set_style()
    epochs = _epochs(rows)
    fig, axes = plt.subplots(
        2,
    ...
    det_ax.plot(
        epochs,
        _series(rows, "tracking_error_storage_detuning"),
        color="#c04f15",
        lw=2.0,
        label="detuning command error",
    )
    det_ax.plot(
        epochs,
        _series(rows, "effective_storage_detuning"),
        color="#5f5f5f",
        ls="--",
        lw=1.6,
        label="residual physical detuning",
    )
    det_ax.axhline(0.0, color="#1f1f1f", lw=0.8, alpha=0.4)
    det_ax.set_xlabel("Epoch")
    det_ax.set_ylabel("Detuning error")
    det_ax.grid(alpha=0.3)
    det_ax.legend(frameon=False)
    _save(fig, path)


def plot_detuning_effective_parameters_vs_epoch(rows: list[dict], path: Path) -> None:
    set_style()
    epochs = _epochs(rows)
    fig, axes = plt.subplots(
        2,
    ...
    det_ax.plot(
        ...
        lw=2.0,
        color="#c04f15",
        label="residual storage detuning",
    )
    det_ax.plot(
        epochs,
        _series(rows, "reference_storage_detuning"),
        lw=1.4,
        color="#c04f15",
        ls="--",
        label="detuning reference",
    )
    det_ax.set_xlabel("Epoch")
    det_ax.set_ylabel("Detuning")
    det_ax.grid(alpha=0.3)
