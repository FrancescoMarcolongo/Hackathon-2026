"""Two-point, no-noise cat-qubit bias optimization.
...
step 2.  The only lifetime-measurement change relative to the step 1 baseline
is that each logical observable is sampled at t=0 and one later time tau.
"""
...
CACHE_ROOT = Path(tempfile.gettempdir()) / "sannino_step02_cache"
...
def evaluate_x(x: np.ndarray, sim_cfg: TwoPointConfig, reward_cfg: RewardConfig) -> dict:
