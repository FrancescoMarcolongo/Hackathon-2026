"""Run step 2 and plot the two-point no-noise convergence figures."""
...
MPL_CONFIG = Path(tempfile.gettempdir()) / "sannino_step02_mplconfig"
...
from two_point_algorithm import BIAS_TOL_REL, TARGET_BIAS, run_step2
...
def plot_reward(rows: list[dict]) -> None:
    ...
    plot_line(ax, epochs, reward, label="two-point incumbent reward", color=COLORS["reward"])
    ax.axhline(0.0, color=COLORS["zero"], linewidth=1.15, linestyle=(0, (4, 4)), label="zero reward")
    style_axes(ax, title="Reward convergence", xlabel="Epoch", ylabel="Reward")
...
def plot_bias(rows: list[dict]) -> None:
    ...
    plot_line(
        ...
        epochs,
        bias,
        label=fr"two-point incumbent $\eta=T_Z/T_X$",
        color=COLORS["bias"],
    )
    style_axes(ax, title="Bias convergence to target", xlabel="Epoch", ylabel=fr"Bias $\eta=T_Z/T_X$")
    ax.set_ylim(0.0, max(float(np.nanmax(bias)), upper) * 1.09)
    set_epoch_ticks(ax, epochs)
    ax.legend(loc="lower right", frameon=True, fancybox=False, framealpha=0.94, edgecolor="#E5E7EB")
...
def plot_lifetimes(rows: list[dict]) -> None:
    ...
    style_axes(ax, title="Validated lifetimes", xlabel="Epoch", ylabel="Lifetime (us)")
    ax.set_yscale("log")
    ax.set_ylim(min(float(np.nanmin(tx)), float(np.nanmin(tz))) * 0.75, max(float(np.nanmax(tx)), float(np.nanmax(tz))) * 1.25)
...
def plot_parameters(rows: list[dict]) -> None:
    ...
    style_axes(ax, title="Control-parameter convergence", xlabel="Epoch", ylabel="Parameter value")
    set_epoch_ticks(ax, epochs)
...
def main() -> None:
    set_slide_style()
    payload = run_step2(verbose=True)
    ...
    final = payload["final"]
    print(
        f"Saved 4 step 2 PNG/PDF figure pairs in {STEP_DIR}. "
        f"Final two-point bias={float(final['bias']):.4g}"
    )
